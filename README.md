...
详细实现可参考[convert_shwd2yolo.py](convert_shwd2yolo.py)，运行方式如下：
  ...
  ```shell
  python convert_shwd2yolo.py --root_dir data/VOC2028
  ```
运行以上命令将在不改变原数据集的前提下，在同级目录生成yolo格式的SHWD数据集。

#### 编写yaml配置文件
配置文件主要包含数据集、数据增强、loss、optimizer、模型结构涉及的相应参数，由于MindYOLO提供yaml文件继承机制，可只将需要调整的参数编写为./configs/yolov7/yolov7-tiny.yaml，并继承MindYOLO提供的原生yaml文件即可，其内容如下：
```
__BASE__: [
...
* 在多卡NPU/GPU上进行分布式模型训练，以8卡为例:
  ...
  ```shell
  apt install openmpi-bin openmpi-common libopenmpi-dev # mpirun

  mpirun --allow-run-as-root -n 8 python train.py --config yolov7-tiny_shwd.yaml --is_parallel True --device_target GPU
  ```

* 在单卡NPU/GPU/CPU上训练模型：

  ```shell
  python train.py --config yolov7-tiny_shwd.yaml
  ```
*注意：直接用yolov7-tiny默认参数在SHWD数据集上训练，可取得AP50 87.0的精度。将lr_init参数由0.01改为0.001，即可实现ap50为89.2的精度结果。*

#### 可视化推理
使用predict.py即可用训练好的模型进行可视化推理，运行方式如下：

```shell
python predict.py --config ./yolov7-tiny_shwd.yaml --weight=/path_to_ckpt/WEIGHT.ckpt --image_path /path_to_image/IMAGE.jpg
```